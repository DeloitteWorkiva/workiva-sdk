...
class AnchorsListResultTypedDict(TypedDict):
    r"""Returns a JSON object with `data` and `@nextLink` properties. `data` contains a list of `Anchor` objects, and `@nextLink` provides the URL to the next set of results. If there are no additional results, `@nextLink` doesn't appear. If the request returns no results at all, `data` contains an empty array."""

    at_next_link: NotRequired[Nullable[str]]
    r"""Pagination link for next set of results"""
    data: NotRequired[List[AnchorTypedDict]]
...
try:
    AnchorsListResult.model_rebuild()
except NameError:
    pass
